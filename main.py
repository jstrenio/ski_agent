...
CSV_FILES = ["ns_qa_p1_100.csv", "ns_qa_p1_200.csv"]
...
for filename in CSV_FILES:
    with open(filename, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            try:
                answers = ast.literal_eval(row.get("answers", "[]"))
            ...
            tokenized_corpus.append(tokenize(thread))
...
def forum_search(query: str) -> str:
    ...
    selected_threads = [threads[i][:2000] for i in top_indices]
...
def wiki_summary(wiki_title: str) -> str:
    ...
    for _ in range(3):
        try:
            summary = wikipedia.page(wiki_title, auto_suggest=False).content[:10000]
            break
        except:
            print('summary failed')
            time.sleep(2)
            try:
                summary = wikipedia.summary(wiki_title, auto_suggest=False)[:10000]
                break
            except:
...
llm_w_tools = llm.bind_tools([forum_search, wiki_search,wiki_summary])
...
def set_plan(state: State):
    ...
    response = plan_chain.invoke({'user_query': user_query})
...
def coordinate_action(state: State):
    response = coordinate_chain.invoke({'messages': state['messages'], 'plan': state['plan']})

    print(f'coordinated action: '
...
def route_action(state: State):
    ...
    tool_calls = (getattr(last_msg, "tool_calls", None) or last_msg.additional_kwargs.get("tool_calls"))
...
def answer_question(state: State):
    response = answer_chain.invoke({'messages': state['messages']})
    return {"messages": [response]}
...
builder.add_node('tools', ToolNode([forum_search, wiki_search, wiki_summary]))
builder.add_edge(START, 'set_plan')
builder.add_edge('set_plan', 'coordinate_action')
builder.add_conditional_edges('coordinate_action', route_action, {'tools': 'tools', 'END': 'answer_question'})
builder.add_edge('tools', 'coordinate_action')
